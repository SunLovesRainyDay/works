% clear: 작업공간의 변수를 모두 지운다/ close all: 그림이 떠 있다면 다 지워라
% clc: 명령창에 있는 것들을 다 지운다.
...
figure;
plot(meas(idx1,1), meas(idx1,2), 'r.'); hold on; %setosa는 빨간점으로
plot(meas(idx2,1),meas(idx2,2), 'go'); hold on; %versicolor는 녹색으로
plot(meas(idx3,1), meas(idx3,2),'bx'); hold on; %virginica는 파란색으로
xlabel('Sepal length');
ylabel('Sepal width');


figure;
plot(meas(idx1,3), meas(idx1,4), 'r.'); hold on; %setosa는 빨간점으로
